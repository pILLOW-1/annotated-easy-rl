...
在策略梯度方法中，我们需要估计优势函数。有多种估计方式：

### k步估计

- 单步估计：高偏差，低方差 — 只使用一步奖励，估计不准确但稳定
- 完整回报估计：无偏差，高方差 — 使用完整回报，准确但不稳定

### GAE：加权平均

GAE通过对不同k步估计进行加权平均来平衡偏差和方差。

取权重为 lambda 的幂次，可以得到简洁的递归计算形式。

### GAE的递归形式

定义TD残差：delta_t = r_t + gamma * V(s_{t+1}) - V(s_t)

则GAE可以递归计算：A_t = delta_t + gamma * lambda * A_{t+1}
...
class GAE:
    ...
    实现GAE公式：A_t = delta_t + gamma * lambda * A_{t+1}
    ...
    def __call__(
        ...
        计算过程：A_t = delta_t + gamma * lambda * A_{t+1}
        """
        # 优势函数表
